cordova.define("kapsel-plugin-apppreferences.AppPreferences", function(require, exports, module) {
// 3.16.0   
...
AppPreferences = function() {
   ...
   * @param {Object} preferencesJSON   JSON object for app preferences screen configuration. If the parameter is provided, then there is no need to call configurePreferencesScreen separately    
   * @example 
   * sap.AppPreferences.showPreferencesScreen( 
   *         function(successMsg) {alert("Succeess: " + successMsg);}, 
   *         function(errMsg) {alert("Error: " + errMsg);} );
   */
   this.showPreferencesScreen = function(successCallback,errorCallback, preferencesJSON) {
        if (preferencesJSON){
            exec(successCallback, errorCallback, "AppPreferences","showPreferencesScreen",[preferencesJSON]);
        }
        else{
   ...
   *
   * @memberof sap.AppPreferences
   * @function configurePreferencesScreen  
